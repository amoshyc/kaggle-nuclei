...
class Trainer(object):
    ...
    def __train(self, ep, msg, pbar):
        batch_size = 10
        msg.update({'loss': 0.0, 'loss_s': 0.0, 'loss_m': 0.0})
        self.model.train()
        for i, (xb, yb) in enumerate(
            ...
            loss_m = self.loss_markers(out[:, 1], yb[:, 1])
            loss = 0.1 * loss_s + loss_m
            loss.backward()
            self.optimizer.step()

            msg['loss'] = (msg['loss'] * i + loss.data[0]) / (i + 1)
            msg['loss_s'] = (msg['loss_s'] * i + loss_s.data[0]) / (i + 1)
            msg['loss_m'] = (msg['loss_m'] * i + loss_m.data[0]) / (i + 1)
            pbar.set_postfix(**msg)
            pbar.update(batch_size)

    def __valid(self, ep, msg, pbar):
        batch_size = 5
        msg.update({'val_loss': 0.0, 'val_loss_s': 0.0, 'val_loss_m': 0.0})
        self.model.eval()
        for i, (xb, yb) in enumerate(
            ...
            msg['val_loss_s'] = (msg['val_loss_s'] * i + loss_s.data[0]) / (
                i + 1)
            msg['val_loss_m'] = (msg['val_loss_m'] * i + loss_m.data[0]) / (
                i + 1)
        pbar.set_postfix(**msg)
    ...
    def __vis(self, ep, msg, pbar):
        ...
        for i, (x, yt, yp) in enumerate(zip(xvis, yvis, yps)):
            ...
            rgb_pm = color.gray2rgb(yp[..., 1])
            rgb_mask = predict.colorize(yp)
            util.make_grid([x, rgb_ti, rgb_tc, rgb_pi, rgb_pm, rgb_mask], 2, 3,
